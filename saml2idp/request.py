...
from . import codex, xml_signing
from .signxml import M2XMLVerifier
...
class RequestProcessor:
    ...
    def parse_request(self):
        ...
        self._xml_tree = lxml.fromstring(self._request_xml)

        sig = self._xml_tree.xpath('/samlp:AuthnRequest/ds:Signature', namespaces=self.NS_MAP)
        if sig:
            self._signed = True
    ...
    def parse_signed(self, x509_cert: str = None):
        ...
        self._xml_tree = M2XMLVerifier().verify(self._xml_tree, x509_cert=x509_cert).signed_xml
        self._signed_data = True

    def _assert_xml_tree(self):
        assert self._xml_tree is not None, self.PARSE_MSG

    def _xpath_xml_tree(self, xpath_statement):
        self._assert_xml_tree()
        return self._xml_tree.xpath(xpath_statement, namespaces=self.NS_MAP)
